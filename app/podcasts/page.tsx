...
function PlatformLinks({
  ...
  const cls =
    tone === "dark"
      ? "border-white/25 text-fg-inverse hover:border-apricot hover:text-apricot"
          ...
          <ul className="flex flex-wrap gap-3">
            {platforms.map((platform) => (
              <li key={platform.name}>
                <a
                  href={platform.href}
                  className="inline-flex items-center rounded-none border-[1.5px] border-white/25 px-6 py-3 text-sm font-medium uppercase tracking-[0.06em] text-fg-inverse transition-colors duration-200 hover:border-apricot hover:text-apricot"
                >
                  {platform.name}
                </a>
              </li>
